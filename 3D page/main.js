...
var yDeg = 0,
    indicator_num = 1,
    indicator_length = page.length,
    w=page[0].offsetWidth, //현재 페이지의 폭을 구함 
    page_angle=0; //각 페이지 4면체의 배치를 위한 각도를 정의하는 함수 

//터치 제스처를 위한 해머 객체 생성 
//매개변수에는 터치 제스처를 적용할 문서객체인 wrapper 변수를 지정 
var hammer = new hammer(wrapper); 
...
function init_page(){
    ...
    for(var i=0; i<page.length;i++){
        page[i].style.transfrom = 'rotateY(' + page_angle + 'deg) translateZ(' + (w/2) + 'px');
        page_angle + =90;
    }
    //wapper 첫 페이지가 정면 보이도록 배치를 초기화 
    wrapper.style.transform = 'translateZ(' + (-w/2) + 'px) rotateY(' + yDeg + 'deg)';
}
